...
export class PageableTableComponent {
  ...
  static allowedPageSizes: number[] = [5, 10, 20];
  ...
  pageButtons() {
    const totalPages =
      Math.ceil(this.config.totalElements / this.config.pageSize) - 1;
    const range = 3;
...
export class TableRow {
  ...
  static of(
    elements: (
      ...
      | number
      | null
      | { value: string | number | null; link: string | string[] }
    )[]
  ) {
...
export class TableColumn {
  ...
  constructor(value: string | number | null, link?: string | string[] | null) {
    ...
    else if (typeof value == 'number') this.value = value.toString();
  ...
  isLinkText() {
    return (
      this.link && !Array.isArray(this.link) && typeof this.link === 'string'
    );
  }
}
